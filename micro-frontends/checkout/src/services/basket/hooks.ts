...
import { useAuth, resolveUserName as resolveAuthUserName } from '@ecommerce-platform/auth-provider';
...
export const ORDER_PLACED_EVENT = 'ecommerce:order:placed';

function getUserName(user: ReturnType<typeof useAuth>['user']): string | null {
  return resolveAuthUserName(user);
}

function resolveUserName(user: ReturnType<typeof useAuth>['user']): string | null {
  return getUserName(user) ?? (env.useMockData ? 'demo@retailmesh.com' : null);
}
  ...
  return useQuery<{ data: Basket } | null>({
    ...
    queryFn: async () => {
      ...
      const response = await checkoutClient.basket.getBasket({
        params: { userName, useMock: env.useMockData },
      });
  ...
  return useMutation({
    ...
    mutationFn: (payload: AddToCartPayload) => {
      ...
      return checkoutClient.basket.addToCart({
        payload: command,
        params: { useMock: env.useMockData },
      });
    },
    onSuccess: (response) => {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      ...
      window.dispatchEvent(
        new CustomEvent(ORDER_PLACED_EVENT, {
          detail: { userName, totalPrice: variables.totalPrice },
        })
      );
