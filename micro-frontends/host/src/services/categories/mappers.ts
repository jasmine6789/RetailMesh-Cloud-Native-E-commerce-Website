...
function getCategoryIcon(name: string): string {
  ...
  if (lowerName.includes('desktop')) return '🖥️';
  if (lowerName.includes('audio') || lowerName.includes('headphone')) return '🎧';
  if (lowerName.includes('accessor')) return '⌨️';
...
function getCategoryImage(name: string): string {
  ...
  if (lowerName.includes('desktop') || lowerName.includes('monitor') || lowerName.includes('display'))
  ...
  if (lowerName.includes('phone') || lowerName.includes('smartphone'))
    return 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80';
